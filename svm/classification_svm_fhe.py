...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert SVM classification to an FHE-friendly pipeline")
    ...
    parser.add_argument("--svm-bits", type=int, default=5, help="Quantization bits for SVM (protects 16-bit limits)")
...
def process_and_print_metrics(
    ...
    # Optional AUC if probabilities are provided
    auc_score = None
...
def main() -> None:
    ...
    y_prob_plain = softmax(clf.decision_function(X_test), axis=1) # Logits to Probabilities
    ...
    extra_meta = {
        ...
        "notes": [
            ...
            "SVM logits converted to pseudo-probabilities via softmax for AUC calculation in plaintext.",
            "Missing classes in execute handles safely via get_all_labels."
        ]
    }

    if args.save_results:
        # Train metrics (for compatibility with old plotting code)
        y_train_pred = clf.predict(X_train)
        y_train_prob = softmax(clf.decision_function(X_train), axis=1)
        ...
        results = {
            ...
            "test_cm_list": [plain_metrics["confusion_matrix"]], # Mirroring for compatibility
        ...
        base_name = f"fhe_svm.{args.feature_type}.{args.scheme}"
