...
"""
usage:
python fhe_xgb_classification.py \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert XGBoost Classification to an FHE-friendly pipeline")
    ...
    # FHE / XGBoost Compile settings
    parser.add_argument("--xgb-bits", type=int, default=5, help="Quantization bits for XGBoost (Default 5)")
    parser.add_argument("--xgb-depth", type=int, default=3, help="Max depth for XGBoost trees (Default 3)")
    parser.add_argument("--xgb-estimators", type=int, default=50, help="Number of trees (Default 50)")
    parser.add_argument("--n-jobs", type=int, default=10, help="Number of CPU threads for training")
...
def main() -> None:
    ...
    # 2. Split & Scale (Keep scaler to align with standard pipeline)
    ...
    print(f"\n🧠 Initializing FHE XGBoost (n_bits={args.xgb_bits}, depth={args.xgb_depth}, trees={args.xgb_estimators}) on {fhe_device.upper()}...")
    clf = XGBClassifier(
        n_bits=args.xgb_bits, 
    ...
    try:
        ...
        print("⚠️ Current concrete-ml XGBoost wrapper doesn't support sample_weight directly. Falling back to unweighted training.")
    ...
    y_prob_plain = clf.predict_proba(X_test) # XGBoost supports direct probability output in plaintext
    ...
    extra_meta = {
        ...
        "notes": [
            ...
            "XGBoost predict_proba used for plaintext AUC.",
    ...
    if args.save_results:
        ...
        results = {
            ...
            "fi_list": fi_list, # Includes Feature Importances for plot compatibility
            "meta": extra_meta,
        }

        base_name = f"fhe_xgb.{args.feature_type}.{args.scheme}"
